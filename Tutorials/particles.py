...
#h             - Toggles a really fake-looking heat wave effect (most noticeable when paused)
...
def init(Screen):
    global View2D, View3D, Spaceship, Plane, CameraRotation, CameraRadius, ParticleSystem, ColorRampShader, fake_heat_effect
    View2D = glLibView2D((0,0,Screen[0],Screen[1]))
    View3D = glLibView3D((0,0,Screen[0],Screen[1]),45,0.1,20)

    CameraRotation = [135,23]
    CameraRadius = 5.0
    fake_heat_effect = False
    ...
    elif particle_system == 3: ParticleSystem = glLibParticleSystem(256**2,32)
    elif particle_system == 4: ParticleSystem = glLibParticleSystem(512**2,32)
    elif particle_system == 5: ParticleSystem = glLibParticleSystem(1024**2,32)
    #Parameters
    ...
    elif particle_system == 3: ParticleSystem.set_particle_size(10.0)
    elif particle_system == 4: ParticleSystem.set_particle_size(50.0)
    elif particle_system == 5: ParticleSystem.set_particle_size(25.0)
...
##    ParticleSystem.point_texture = whatever texture you want.
    ...
    #For a really fake looking heat wave effect
    ColorRampShader.user_variables("uniform float time;uniform bool fake_heat_effect;")
    ColorRampShader.uv_transform("if (fake_heat_effect) { uv.x += 0.001*sin((time+uv.y)*360.0); }")
...
time = 0.0
def GetInput():
    global CameraRadius, paused, rotating, time, fake_heat_effect
    ...
    for event in pygame.event.get():
        ...
        if event.type == KEYDOWN:
            ...
            elif event.key == K_h: fake_heat_effect = not fake_heat_effect 
...
##    Plane.draw()
    ...
    ColorRampShader.pass_bool("fake_heat_effect",fake_heat_effect)
