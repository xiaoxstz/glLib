...
##    FloorTexture = glLibTexture2D("data/floor2.png",[0,0,512,512],GLLIB_RGBA)
    FloorTexture = glLibTexture2D("data/Image1.png",[0,0,4,4],GLLIB_RGBA)
    FloorTexture = glLibTexture2D("data/reconstruct.png",GLLIB_ALL,GLLIB_RGB)
    ...
    Shader = glLibShader()
    #Custom filtering shader.  texture2Dcf() Maintains continuous 1st derivative
    #texture2D() uses the current filtering value.
##    Shader.render_equation("color.rgba = texture2Dcf(tex2D_1,vec2("+str(float(FloorTexture.width))+"),uv).rgba;")
    Shader.render_equation("color = texture2Dbc(tex2D_1,vec2("+str(float(FloorTexture.width))+"),uv,-0.5);")
    ...
    for event in pygame.event.get():
        if event.type == QUIT: quit(); pygame.quit(); sys.exit()
            ...
            if   event.key == K_ESCAPE: return False
                ...
                elif filtering == GLLIB_FILTER: filtering = "CUSTOM SHADER"
...
def Draw(Window):
    ...
    #Draw the panels
    if filtering == "CUSTOM SHADER": glLibUseShader(Shader)
    ...
    if filtering == "CUSTOM SHADER": glLibUseShader(None)
