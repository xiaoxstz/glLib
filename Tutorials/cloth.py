...
#Because the mesh is rectangular, one would normally use many quad
...
#quadrilateral cannot be seen because it has no area, so the
#appearance of a simple surface is preserved.  You can see it in
#line mode though as the lines that cross the mesh.  Note that if
#the cloth is caught between two objects, the cloth will get
#confused.  Best to keep the objects from intersecting.  
...
def init(Screen):
    ...
    mouse_obstacle_pos = [-0.4,0.6,-0.4]
    ...
    #The radius is slightly less than the collision
    #object.  The collision detection works per-
    #vertex, so individual polygons can still pass
    #through.  By making the collision object larger
    #than the actual object, this effect is lessened
    #or eliminated.
    sphere1 = glLibSphere(0.399,32)
    #Mouse-moveable obstacle object (for drawing)
    #Again, the radius is less.
...
def SetLights():
    ...
    Light1.set_diffuse([4.0,2.0,2.0])
    Light1.set_specular([4.0,2.0,2.0])
    ...
    Light2.set_diffuse([2.0,2.0,4.0])
    Light2.set_specular([2.0,2.0,4.0])
    ...
    Light3.set_diffuse([2.0,4.0,2.0])
    Light3.set_specular([2.0,4.0,2.0])
...
def GetInput():
    ...
    if mpress[2]:
        ...
        #Keep the obstacle inside the bounding box (use 0.999 instead of 1.0)
        mouse_obstacle_pos = clamp(mouse_obstacle_pos,-0.999,0.999)
        #Keep the user from moving the mouse obstacle to intersect the stationary obstacle
        diff_vec = vec_subt(mouse_obstacle_pos,[0.0,0.4,0.0])
        if length(diff_vec) < 0.6:
            diff_vec[1] = 0.0
            mouse_obstacle_pos = vec_add([0.0,0.6,0.0],sc_vec(0.6,normalize(diff_vec)))
        #Move the obstacle to this position
        Cloth.change_obstacle("mouse",mouse_obstacle_pos)
