...
#o             - Switches among a spaceship object, a torus object, a blob-sphere object, a disco ball, and a ring
#UP            - Rotate light up
...
def init(Screen):
    ...
    global Spaceship, DiscoBall, TriSphere, Floor, FloorNoTexture, Cylinder, object, EnvCube, CausticGrid, Cubemap
    global ObjectRotation, ObjectPosition, CameraRotation, CameraRadius, rotating, rotatingcounter
    ...
    global causticparticlesize, causticparticlebrightness, eta, chromatic_aberration, refraction_reflection
    using_fbo = False
    shadowmapres = 512
    cubemapres = 128
    grid_sc = 0.8
...
##    causticparticlebrightness = 0.18
##    causticparticlesize = 6
##    causticparticlebrightness = 0.045
    causticparticlesize = 9
    causticparticlebrightness = 0.02
    ...
    try:
        ...
        caustic_fbo.add_render_target(1,filtering=GLLIB_FILTER)
    except:
        raise glLibError('causticmapping.py: "Error: FBOs could not be created"')

    Spaceship = glLibObject("data/objects/Spaceship.obj",GLLIB_FILTER,GLLIB_MIPMAP_BLEND)
    Spaceship.build_vbo()
    TriSphere = glLibObject("data/objects/trisphere.obj",GLLIB_FILTER,GLLIB_MIPMAP_BLEND)
    TriSphere.build_vbo()
    ...
    FloorNoTexture = glLibPlane(5,(0,1,0),uv_tile=10)
    ...
    //to use the normalmap for drawing the object only (not computing caustic map) enable this line and line 392
    ...
    //to self-shadow the object, enable this line and the line 395
...
##    print("No errors to report with object drawing shader (causticmapping.py)!")
    ...
    color.rgb += add_caustics(tex2D_3,depth_coord_1);
...
def GetInput():
    global CameraRadius, ObjectRotation, rotating, rotatingcounter, object, refraction_reflection, chromatic_aberration, using_fbo, eta
    global causticposmaps, causticnormmaps, causticmap
    ...
    for event in pygame.event.get():
        if event.type == QUIT: quit(); pygame.quit(); sys.exit()
            ...
            if event.key == K_ESCAPE: return False
                ...
                chromatic_aberration = not chromatic_aberration
    ...
    ObjectRotation = list(map(lambda num:num%360.0,ObjectRotation))
...
def draw_floor():
    ...
    if depthmap:
        glLibUseDepthMaps([[depthmap,proj,view]],1,2,CausticPhongShader,lambda:0)
    if causticmap:
...
def draw_object_simple():
    ...
    if object == 1:
        ...
        TriSphere.draw_vbo(withmaterials=False)
    elif object == 4:
        glPushMatrix()
        glScalef(0.3,0.3,0.3)
        DiscoBall.draw_vbo()
...
def Draw(Window):
    ...
    glLibUpdateCubeMap(Cubemap,CubemapView,ObjectPosition,draw_reflectees)

    Window.clear()
    glLibUseShader(None)
    depthmap,proj,view = glLibMakeDepthMap(Light1,ShadLightView,ObjectPosition,depthmap,draw_object_simple,filtering=GLLIB_FILTER)

    #Set up the receiving, refracting, and reflecting draw functions.  If there is more than one layer,
    #none of these functions can have ANY texture bindings to texture target 1!
    ...
    #Generate positionmap and normalmap from the light's perspective (either works,
    #but using the FBO is much more efficient, because it uses only 1/2 as many passes).
    if using_fbo:
        causticposmaps,causticnormmaps = glLibCausticPositionNormalMapFBO(posnorm_fbo1,posnorm_fbo2,Light1,CaustLightView,ObjectPosition,
                                                                          layers,CausticPositionNormalShader,\
                                                                          draw_receivers_func,draw_refractors_func,draw_reflectors_func)
    else:
        causticposmaps,causticnormmaps = glLibCausticPositionNormalMap(Light1,CaustLightView,ObjectPosition,layers,CausticPositionNormalShader,\
                                                                       draw_receivers_func,draw_refractors_func,draw_reflectors_func,\
                                                                       causticposmaps,causticnormmaps,filtering=False,precision=8)

    #Generate causticmap from light's perspective
    if chromatic_aberration:
        #   Data for RGB colors and refractive index.  
        colordata = [  [[1.0*causticparticlebrightness,0.0,0.0],clamp(eta+0.10*(1.0-eta),0.25,1.0)],
                       [[0.0,1.0*causticparticlebrightness,0.0],clamp(eta+0.00*(1.0-eta),0.25,1.0)],
                       [[0.0,0.0,1.0*causticparticlebrightness],clamp(eta-0.10*(1.0-eta),0.25,1.0)]  ]
    else:
        #   Simple white color and refractive index
        colordata = [  [[1.0*causticparticlebrightness,1.0*causticparticlebrightness,1.0*causticparticlebrightness],eta]  ]
    if using_fbo:
        causticmap = glLibCausticMapFBO(caustic_fbo,Light1,CaustLightView,ObjectPosition,object_transform,CausticmapGeneratorShader,CausticGrid,grid_sc,\
                                        causticparticlesize,colordata,causticposmaps[:1],causticnormmaps[:1])
    else:
        causticmap = glLibCausticMap(Light1,CaustLightView,ObjectPosition,object_transform,CausticmapGeneratorShader,CausticGrid,grid_sc,causticparticlesize,\
                                     colordata,causticposmaps[:1],causticnormmaps[:1],causticmap,filtering=GLLIB_FILTER)
    ...
    #To draw the object using normalmaps, enable the line below as well as the indicated line in ObjectShader
    #ObjectShader.pass_texture(Normalmap,2)
    ObjectShader.pass_float("eta",eta)
    #If shadowing in ObjectShader, enable the line below as well as the indicated line in ObjectShader
    #glLibUseDepthMaps([[depthmap,proj,view]],1,3,ObjectShader,object_transform)
    glPushMatrix()
    draw_object()
    glPopMatrix()
    ...
    View2D.set_view()
    glDisable(GL_LIGHTING)
    glLibDrawScreenQuad(texture=causticnormmaps[0])
    glEnable(GL_LIGHTING)
