...
from glLibObjects import glLibDrawScreenQuad,glLibGrid2D,glLibGrid2DMesh
...
##        self.
        ...
        self.update_framebuffer.add_render_target(3,precision=8) #for debugging
...
##        self.update_shader.save_vertex()
    ...
    def move_obstacle(self,id,newpos):
        ...
        self.glLibInternal_calculate_obstacles()
    def change_obstacle(self,id,newpos,param=GLLIB_OLD):
        type,oldparam = self.collidable[id][-1]
        if param==GLLIB_OLD: param = oldparam
        self.collidable[id] = [newpos[0],newpos[1],newpos[2],[type,param]]
        self.glLibInternal_calculate_obstacles()
    ...
    def glLibInternal_calculate_obstacles(self):
        ...
        for obstacle in self.collidable.values():
            ...
            position_type = list(map(lambda num:0.5*((num+1.0)/2.0),position))
            type,param = obstacle[-1]
            if type == GLLIB_OBSTACLE_POLYGONAL:
                position_type[0] += 0.5
                color1 = [position_type[0],position_type[1],position_type[2],0.0]
    ...
    def glLibInternal_use_collision_shader(self):
        ...
        if self.num_obstacles > 0:
            ...
            if self.polygonal_object_textures != None:
                self.collision_shader.pass_texture(self.polygonal_object_textures[0],6)
                self.collision_shader.pass_texture(self.polygonal_object_textures[1],7)
                self.collision_shader.pass_float("texture_size",self.polygonal_object_textures[0].width)
...
##        glDisable(GL_LIGHTING)
##        glLibSelectTexture(self.collision_framebuffer.get_texture(3))
####        glLibSelectTexture(self.polygonal_object_texture)
##        glLibDrawScreenQuad(texture=True)
##        glEnable(GL_LIGHTING)
